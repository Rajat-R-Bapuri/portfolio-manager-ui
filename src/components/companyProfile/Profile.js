...
class Profile extends React.Component {
    ...
    componentWillMount() {
        fetch(process.env.REACT_APP_BACKEND_API + `/stocks/${this.props.symbol}/company`)
                ...
                <div>
                    <Grid container spacing={3}>
                        <Grid item className={mClasses.heading}>
                            ...
                            <Link className={`${mClasses.webLink}`}
                        ...
                        <Grid item>
                            <Paper elevation={3} className={mClasses.cardsCommon}>{this.state.data["description"]}</Paper>
                        </Grid>


                        <Grid item xs={4}>
                            <Paper elevation={3} className={`${mClasses.cardsCommon} ${mClasses.fixedCardHeight}`}>
                    ...
                    <div className={mClasses.tagButtons}>
                        {
                            this.state.data["tags"].map((value, index) => {
                                return (
                                    <Button style={{ margin: "10px" }} key={value} variant="contained" color="primary">
                                        {value}
                                    </Button>
                                );
                            })
